estimate_simulate_usa <- function(dt         ,
  ...
  # Import the data
  calib_dt          <- data.table(read_fst(file.path(out_path, paste0("_Master_B_" ,
                                                                      platform     ,
                                                                      ".fst"        ))))
  # Get the minimum iteration in use
  minimum_iterations <- data.table(read_fst(file.path(out_path, paste0( 
                                                      "_Master_A_" ,
                                                      platform     , 
                                                      ".fst"))))
  ...
  calib_dt[, pen_A_beg := pen_A_only_beg + pen_AB_beg, ]
  
  # Order the data.table
  calib_dt <- calib_dt[order(seed, iteration, country, period)]
  
  # Add lags
  calib_dt[, lag_pen_A           := shift(pen_A      , n=1L, type = "lag"), by = .(seed, iteration, country)]
  calib_dt[, lag_pen_A_only      := shift(pen_A_only , n=1L, type = "lag"), by = .(seed, iteration, country)]
  calib_dt[, lag_pen_AB          := shift(pen_AB     , n=1L, type = "lag"), by = .(seed, iteration, country)]
  calib_dt[, lag_pen_A_beg       := shift(pen_A_beg  , n=1L, type = "lag"), by = .(seed, iteration, country)]
  calib_dt[, lag_y_churn         := shift(y_churn    , n=1L, type = "lag"), by = .(seed, iteration, country)]
  
  # Calculate the probability of adopting A
  calib_dt[, prob_A_calc := (pen_A_only_beg + pen_AB_beg - lag_pen_A_only - lag_pen_AB) /
              (1 - (lag_pen_A_only + lag_pen_AB)), ]
  
  # Calculate the LHS variable
  calib_dt[, lhs   := log(prob_A_calc / (1 - prob_A_calc)), ]
  
  # Remove iterations where the lhs becomes infinite
  calib_dt[, inf_flag := (is.infinite(lhs) | is.nan(lhs)),]
  calib_dt[, max_inf_flag := max_na(inf_flag), by = .(seed, iteration)]
  calib_dt_ninf <- calib_dt[max_inf_flag != 1,,]
  
  # Set the order of the variables in the regression
  calib_dt_ninf <- calib_dt_ninf[order(seed, iteration, country, period),,]
  
  # Create binary competitor variable ---------------------------------------
  
  calib_dt_ninf[pen_B_beg >= 0.25 * pen_A_beg , comp_var := TRUE  , ]
  # Set the binary competitor variable to zero if missing
  calib_dt_ninf[is.na(comp_var), comp_var := FALSE , ]
  
  # Copy the dataset
  sutil <- copy(calib_dt_ninf)
  ...
  for (scenario in c("actual", "cf")) {
    ...
    calib_dt[, iteration_num := .GRP, by = .(seed, iteration)]
    
    for (i in 1:(nrow(calib_dt[iteration_num == 1 & country == 1]) - 1)) {
  ...
  calib_dt_ninf <- calib_dt_ninf[!(seed %in% set_NA[, seed, ] &
                                       iteration %in% set_NA[, iteration, ])]
  
  
  if ((all(actual_dt[, .(pen_A_beg), ] - calib_dt_ninf[, .(pen_A_beg), ] < 0.1^10) == FALSE)) {
  ...
  actual_cf_athey_m <- actual_cf_athey_m[, .(iteration , country      , period       ,
                                             pen_A_beg , pen_A_beg_cf , cf_pen_A_bin ,
                                             u_A       , u_B          , beta         ,
                                             gamma     , MSE          , seed          )]
  
  # Save the datasets
  save_fst(actual_cf_athey_m , paste0("5.0001_"       ,
                                      platform        , 
                                      "_start_multi_" ,
                                      start_multi_pen  ) , 
           out_path)
